...
class Visitor {
  ...
  visitModule(ast, level) {
    ...
    this.conflictModels = [];
    this.visitConflictModel(ast.conflictModels);
  ...
  visitType(ast, level, env) {
    if (ast.type === 'array') {
      ...
      let moduleName = '';
      let modelName = _name(ast);
      if (ast.moduleName && this.conflictModels.indexOf(modelName) > -1) {
        moduleName = `${this.imports[ast.moduleName].namespace}.Models`;
      ...
      ast.path.forEach((item, index) => {
        if (index === 0) {
          ...
          modelName += `${_upperFirst(_name(item))}`;
        }
      });
      if (this.conflictModels.indexOf(modelName) > -1) {
        this.emit(`${this.imports[moduleName].namespace}.Models.`);
      }
      this.emit(`${modelName}`);
      ...
      const modelName = _upperFirst(_name(ast));
      if (this.conflictModels.indexOf(modelName) > -1) {
  ...
  visitConstructModel(ast, level, env) {
    ...
    if (ast.propertyPath && ast.propertyPath.length > 0) {
      let modelName = ast.propertyPath.map((item) => {
        return _name(item);
      }).join('.');
      if (ast.inferred && ast.inferred.moduleName && this.conflictModels.indexOf(modelName) > -1) {
...
`);
  ...
  async visitUsing() {
    const usedModels = this.usedExternModel;
    for(var item in this.imports) {
      if (this.imports.hasOwnProperty(item)) {
        this.emit(`using ${this.imports[item].namespace};\n`);
        if ([...usedModels.get(item)].length > 0){
          this.emit(`using ${this.imports[item].namespace}.Models;\n`);
        }
      }
    }
  }

  async visitConflictModel(conflicts) {
    for(const key of conflicts.keys()) {
      const model = key.split(':')[1];
      if (this.conflictModels.indexOf(model) < 0) {
        this.conflictModels.push(model);
      }
    }
  }
}
